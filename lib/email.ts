...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'bostonautomotores@hotmail.com'
 ...
 */
export async function sendContactNotificationEmail(lead: ContactEmailData): Promise<{ success: boolean; error?: string }> {
  const { render } = await import('@react-email/render')
