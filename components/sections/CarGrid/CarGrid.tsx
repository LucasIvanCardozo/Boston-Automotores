...
export default function CarGrid({ cars }: CarGridProps) {
  if (!cars || cars.length === 0) {
    return (
      <div className={styles.empty}>
        ...
        <p className={styles.emptyText}>Probá modificando los filtros o contactanos para ayudarte a encontrar tu próximo auto.</p>
      </div>
    )
  }
  ...
  return (
    <div className={styles.grid}>
      {cars.map((car, index) => (
        <motion.div key={car.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }}>
          <CarCard car={car} />
        </motion.div>
      ))}
    </div>
